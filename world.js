...
function makeRecord(i){
  ...
  return { id:`JQ-${String(i+1).padStart(4,"0")}`, ts:`T-${days}d · ${hh}:${mm}Z`, instrument:pick(instruments), setup:pick(setups), r:`${r}R`, note:pick(notes) };
}
...
function makeFrameGeometry(w,h){ return new THREE.PlaneGeometry(w,h,1,1); }
...
function makeBorderMaterial({color=0x22D3EE,opacity=0.20,thickness=0.028,glow=0.60}={}){
  return new THREE.ShaderMaterial({
    ...
    fragmentShader:`
      uniform vec3 uColor; uniform float uOpacity; uniform float uThickness; uniform float uGlow;
      varying vec2 vUv;
...
function makeTextTexture({title,body,w=1024,h=640}){
  ...
  g.lineWidth=2; g.beginPath(); g.moveTo(56,116); g.lineTo(w-56,116); g.stroke();
  g.fillStyle="rgba(245,247,255,0.78)";
  g.font="700 30px ui-monospace, Menlo, Consolas, monospace";
  const lines=body.split("\n");
  let y=160;
  for(const line of lines){ g.fillText(line,56,y); y+=44; }
...
export function createWorld(canvas,{onHoverFragment,onSelectRecord}={}){
  ...
  latticeA.position.copy(origin); latticeA.rotation.y=yaw; scene.add(latticeA);

  const latticeB=buildLattice({size:260,step:12,color:0x6D28D9,opacity:0.06});
  latticeB.position.copy(origin); latticeB.rotation.y=yaw; scene.add(latticeB);
  ...
  for(let i=0;i<manifest.length;i++){
    ...
    border.userData.rec={id:manifest[i].id,ts:"manifest",instrument:"JarvQuant",setup:manifest[i].title,r:"—",note:manifest[i].body.replace(/\n/g," ")};
    ...
    border.add(plate); border.add(inner);
    const x=(i%2===0)?-8.6:8.8;
    const y=1.4-(i*0.6);
    const z=12-(i*18);
    border.position.set(x,y,z);
    border.rotation.y=(i%2===0)?0.35:-0.35;
    frameGroup.add(border);
    frames.push(border);
  }

  // Random record frames (avoid gallery corridor)
  ...
  for(let i=0;i<exhibits.length;i++){
    ...
    const inner=new THREE.Mesh(makeFrameGeometry(5.05,2.92),new THREE.MeshBasicMaterial({color:0x0a0f18,transparent:true,opacity:0.90}));
    inner.position.z=0.001;
  ...
  function setChapter(name){ state.chapter=name; } // IMPORTANT: no snapping
  ...
  function selectObject(obj){
    ...
    if(onSelectRecord){
      if(rec?.id?.startsWith("EX-")){
        ...
        onSelectRecord({title:`${rec.id} · ${rec.instrument} · ${rec.setup}`, body:`${rec.ts} · ${rec.r}\n\n${rec.note}`});
      } else {
        onSelectRecord(null);
      }
    }
  ...
  function getBeaconScreenspace(){
    return beacons.map(b=>{
      ...
      return { id:b.id, title:b.title, body:b.body, x:(v.x*0.5+0.5)*window.innerWidth, y:(-v.y*0.5+0.5)*window.innerHeight, z:v.z };
    });
  }
